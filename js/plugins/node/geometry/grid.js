...
SceneJS.Types.addType("geometry/grid", 
{
    ...
    {
        this._translate = this.addNode({ type:"translate", y:-1 });
        ...
        if (params.pos) { this.setPos(params.pos); }
        if (params.size) { this.setSize(params.size); }
    },

    setPos: function (pos) { this._translate.setXYZ(pos); },
    setSize: function (size) { this._scale.setXYZ(size); }
});
