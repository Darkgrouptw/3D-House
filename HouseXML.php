...
class HouseXML
{
	...
	function ReadText()
	{
		...
		for($i = 0; $i < $layers->length; $i++)
		{
			...
			for($j = 0; $j < $elements->length; $j++)
			{
				...
				$flagsWrapStr="					type: \"flags\",\n 					flags: { transparent:false },\n\n 								nodes:\n 					[{\n".$nameDoubleWrapStr."				}]\n";
				$this->RenderList = $this->RenderList.$flagsWrapStr."\n		},\n";
			}
		}
	...
	private function MatrixBind(&$str, $transform)
	{
		...
		$ans = $this->MatrixOperation($this->MatrixOperation($this->MatrixOperation($this->MatrixOperation($rotate_m_X,$rotate_m_Y),$rotate_m_Z),$scale_m),$translate_m);
		$str = "						type: \"matrix\",\n						elements:[". $ans[0];
		for($i = 1; $i < 16; $i++)
			$str = $str.",".$ans[$i];
		$str = $str."],\n\n";
